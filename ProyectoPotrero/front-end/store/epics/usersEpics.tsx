...
const searchUsersEpic: Epic<UsersAction, UsersAction, IState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      let url = `https://proyectospotrerodigital.backend.aptugo.app/api/users/search/`
...
const loadUsersEpic: Epic<UsersAction, UsersAction, IState> = (action$, state$) => {
  ...
  return action$.pipe(
    ...
    switchMap((action) => {
      let url = `https://proyectospotrerodigital.backend.aptugo.app/api/users/`
...
const addUsersEpic: Epic<UsersAction, UsersAction, IState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      return from(axios.post(`https://proyectospotrerodigital.backend.aptugo.app/api/users/`, data, config)).pipe(
...
const removeUsersEpic: Epic<UsersAction, UsersAction, IState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap((action) =>
      from(axios.delete(`https://proyectospotrerodigital.backend.aptugo.app/api/users/${action.payload._id}`)).pipe(
...
const editUsersEpic: Epic<UsersAction, UsersAction, IState> = (action$, state$) =>
  action$.pipe(
    ...
    mergeMap((action) => {
      ...
      return from(axios.put(`https://proyectospotrerodigital.backend.aptugo.app/api/users/${action.payload._id}`, data, config)).pipe(
